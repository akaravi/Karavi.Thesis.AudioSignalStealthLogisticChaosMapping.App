...
| 2026-05-22 | 11:15 | 67 | رفع دکمه «نهان‌نگاری جدید»: فعال وقتی ورودی مخفی است؛ چیدمان Column به‌جای Stack (کلیک پشت اسکرول)؛ اسکرول به بالا؛ `_busy` در finally؛ ضبط مجدد ورودی را نشان می‌دهد | `embed_screen.dart`, `tab_scroll_body.dart` |
| 2026-05-22 | 11:05 | 66c | ادامهٔ بررسی: `AudioStegApp`→`AudioSteganoApp`؛ حذف `kotlin/com/karavi/thesis/` خالی؛ اسکن repo (۰ filename باقی)؛ MATLAB/docs/publish/scripts تأیید؛ flutter test ۳۷/۳۷ | `lib/main.dart`, `Cursor.01.plan.md` (ساختار مخزن) |
| 2026-05-22 | 11:02 | 66b | تکمیل تغییر نام: `.iml` فایل‌ها، پوشهٔ kotlin خالی، خروجی‌های `publish/` (aab/apk/bin/exe/dll/pdb/json) و خود اسکریپت rename — هیچ filename با `Steg`/`steg` باقی نماند. Flutter analyze + dotnet build مجدد سبز | `audio_stegano_app.iml`, `audio_stegano_app_android.iml`, `publish/cafebazaar/AudioStegano_*`, `publish/flutter/android/AudioStegano_*`, `publish/dotnet/win-x64/AudioStegano.Desktop/*`, `scripts/_rename_to_stegano.ps1` |
| 2026-05-22 | 10:55 | 66 | تغییر نام جامع `Steg` → `Stegano` (همه شناسه‌ها، نام پروژه‌ها، sln/csproj، پوشه‌ها). حفظ `Stego`, `Stega`, `Steganography`. Flutter analyze + test (۳۷/۳۷) سبز؛ dotnet build + test (۵/۵) سبز؛ golden‌های Cafe Bazaar به‌روز | `scripts/_rename_to_stegano.ps1`, `AudioStegano.sln`, `src/audio_stegano_app/`, `src/audio_stegano_desktop/`, `*.csproj`, `*.cs`, `*.xaml`, `*.dart`, `pubspec.yaml`, `*.ps1`, docs |
