...
class _EmbedScreenState extends ConsumerState<EmbedScreen> {
  ...
  /// True while a `_toggleRecord` invocation is in flight. Set synchronously
  /// at the entry of `_toggleRecord` to avoid races on rapid taps.
  bool _busy = false;
  bool _processing = false;
  bool _verifying = false;
  StegoMode _mode = StegoMode.overTheAir;
  WavFile? _stego;
  ...
  Future<void> _stopAndProcess() async {
    ...
    // Cancel amplitude FIRST so we don't get a tick from a closed stream.
    await _cancelAmp();
    ...
    // Pre-flight capacity check for digital LSB mode — fail fast with a
    // clear message instead of a confusing ArgumentError from the codec.
    if (_mode == StegoMode.digital) {
      final required = TextCodec.requiredBits(text);
  ...
  Future<void> _saveStego() async {
    ...
    try {
      ...
      targetPath = '${dir.path}${Platform.pathSeparator}'
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            if (result != null) _buildMetricsBlock(s, theme, result, durationSec),
            const SizedBox(height: 12),
            Wrap(
              spacing: 8,
              runSpacing: 8,
  ...
  Widget _buildMetricsBlock(
      AppStrings s, ThemeData theme, EmbedRunResult result, double durationSec) {
    final chips = <Widget>[
      _metricChip(theme, Icons.timer_outlined, s.duration,
          '${durationSec.toStringAsFixed(2)} s'),
      _metricChip(theme, Icons.token_outlined, s.bitsEmbedded,
          '${result.bitsEmbedded}'),
      _metricChip(theme, Icons.storage_outlined, s.capacity,
          '${result.capacityBits}'),
      _metricChip(
          theme,
      ...
      if (result.snrDb != null && result.snrDb!.isFinite)
        _metricChip(theme, Icons.graphic_eq, s.snrLabel,
            result.snrDb!.toStringAsFixed(2)),
      if (result.psnrDb != null && result.psnrDb!.isFinite)
        _metricChip(theme, Icons.equalizer, s.psnrLabel,
            result.psnrDb!.toStringAsFixed(2)),
    ];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(s.qualityMetrics,
