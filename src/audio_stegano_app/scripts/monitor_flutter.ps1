# Tails Flutter session + run logs while testing. Run from audio_stegano_app/.
param(
    [int]$PollSeconds = 2
)
...
while ($true) {
    $proc = Get-Process -Name 'audio_stegano_app' -ErrorAction SilentlyContinue
    $alive = $null -ne $proc
    ...
    if (-not $alive) {
        $stamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
        $msg = "[$stamp] PROCESS EXIT: audio_stegano_app not running"
