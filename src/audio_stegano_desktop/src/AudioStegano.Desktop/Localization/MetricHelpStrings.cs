...
{
    public string HelpClose => T("بستن", "Close", "إغلاق", "Fermer");

    public string MetricHelpTapHint => T(
        "برای توضیح هر متریک، روی آن کلیک کنید.",
        "Click a metric chip for a full explanation.",
        "انقر على أي مقياس لعرض الشرح الكامل.",
        "Cliquez sur une pastille pour l’explication complète.");

    public string MetricHelpTitle(EmbedMetricKind kind) => kind switch
    {
        EmbedMetricKind.Duration => T(
            "مدت زمان صوت", "Audio duration", "مدة الصوت", "Durée audio"),
        EmbedMetricKind.BitsEmbedded => T(
            "بیت‌های جاسازی‌شده", "Bits embedded", "البتات المدمجة", "Bits intégrés"),
        EmbedMetricKind.Capacity => T(
            "ظرفیت نهان‌نگاری", "Embedding capacity", "سعة الإخفاء", "Capacité d’intégration"),
        EmbedMetricKind.Utilization => T(
            "بهره‌وری ظرفیت", "Capacity utilisation", "استخدام السعة", "Utilisation de la capacité"),
        EmbedMetricKind.MsgBitLength => T(
            "طول پیام (بیت)", "Message length (bits)", "طول الرسالة (بت)", "Longueur du message (bits)"),
        EmbedMetricKind.Snr => T(
            "نسبت سیگنال به نویز (SNR)", "Signal-to-noise ratio (SNR)",
            "نسبة الإشارة إلى الضوضاء (SNR)", "Rapport signal/bruit (SNR)"),
        EmbedMetricKind.Psnr => T(
            "نسبت اوج سیگنال به نویز (PSNR)", "Peak signal-to-noise ratio (PSNR)",
            "ذروة نسبة الإشارة إلى الضوضاء (PSNR)", "Rapport signal/bruit de crête (PSNR)"),
        EmbedMetricKind.Ber => T(
            "نرخ خطای بیت (BER)", "Bit error rate (BER)",
            "معدل خطأ البت (BER)", "Taux d’erreur binaire (BER)"),
        EmbedMetricKind.Npcr => T(
            "نرخ تغییر نمونه‌ها (NPCR)", "Number of changed samples (NPCR)",
            "نسبة العينات المتغيرة (NPCR)", "Taux d’échantillons modifiés (NPCR)"),
        EmbedMetricKind.Uaci => T(
            "شدت تغییر میانگین (UACI)", "Uniform average change intensity (UACI)",
            "شدة التغيير المتوسط الموحّد (UACI)", "Intensité moyenne uniforme de changement (UACI)"),
        _ => string.Empty,
    };

    public string MetricHelpBody(EmbedMetricKind kind) => kind switch
    {
        EmbedMetricKind.Duration => T(
